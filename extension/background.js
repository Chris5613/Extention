...
const DEFAULT_SETTINGS = {
  ...
  combinedMode: true,
  combinedEmail: "combined@unity-network",
  combinedLabel: "Unity Network",
...
function getAccountToken(acc) {
  if (acc.manualToken && acc.manualToken.trim()) {
    return {
      token: acc.manualToken.trim(),
      source: "manual",
    };
  }

  if (acc.autoToken && acc.autoToken.trim()) {
    return {
      token: acc.autoToken.trim(),
      source: "auto",
    };
  }
...
async function upsertAutoAccounts(tokens) {
  ...
  for (const meta of metas) {
    ...
    const label = existing
      ? existing.label === existing.email ||
        !existing.label ||
        existing.label === "Unity Account" ||
        existing.label === "Account 1"
        ? labelFallback
        : existing.label
      : labelFallback;
  ...
  return {
    email: metas.map((m) => m.email).find(Boolean) || null,
...
function getTzOffsetMinutes(utcDate, timeZone) {
  try {
    ...
    fmt.formatToParts(utcDate).forEach((p) => {
...
function buildPayload(allocations, balanceMicros, email) {
  ...
  const todayLocal = `${now.getFullYear()}-${pad2(now.getMonth() + 1)}-${pad2(
    now.getDate()
  )}`;
  ...
  return {
    ...
    balance_usd:
      balanceMicros != null ? Number((balanceMicros / 1e6).toFixed(6)) : null,
...
// ───────────────────────────────────────────────────────────────
// Build combined account payload
// ───────────────────────────────────────────────────────────────

function buildCombinedPayload(enabledAccounts, settings, triggeredBy, freshIds) {
  const perAccount = enabledAccounts.map((a) => a.lastFullPayload).filter(Boolean);

  const totalToday = perAccount.reduce((s, p) => s + Number(p.total_usd || 0), 0);
  const totalLifetime = perAccount.reduce(
    (s, p) => s + Number(p.lifetime_usd || 0),
    0
  );
  const totalBalance = perAccount.reduce(
    (s, p) => s + Number(p.balance_usd || 0),
    0
  );

  const allocationCount = perAccount.reduce(
    (s, p) => s + Number(p.allocation_count || 0),
    0
  );

  const deviceCount = perAccount.reduce(
    (s, p) => s + Number(p.device_count || 0),
    0
  );
...
async function syncOneAccount(accountId) {
  ...
  if (!token) {
    accounts[idx] = sanitizeAccount({
      ...
      lastSyncError:
        "No auth token. Open https://manage.unitynodes.io and sign in to this account, or paste a token in Settings.",
    });
...
async function performSync({ triggeredBy = "manual", accountId = null } = {}) {
  ...
  if (toSync.length === 0) {
    const err =
      "No accounts configured. Open https://manage.unitynodes.io and sign in to capture a token, or add a manual token in Settings.";
  ...
  const allEnabled = (refreshed.accounts || [])
    .map(sanitizeAccount)
    .filter((a) => a.enabled !== false);

  const latestPerAccount = allEnabled.map((a) => a.lastFullPayload).filter(Boolean);

  const combinedPayload = buildCombinedPayload(allEnabled, s, triggeredBy, freshIds);

  const totalToday = latestPerAccount.reduce(
    (sum, p) => sum + Number(p.total_usd || 0),
    0
  );

  const totalLifetime = latestPerAccount.reduce(
    (sum, p) => sum + Number(p.lifetime_usd || 0),
    0
  );

  const totalBalance = latestPerAccount.reduce(
    (sum, p) => sum + Number(p.balance_usd || 0),
    0
  );
  ...
  const overallStatus =
    errCount === 0 ? "ok" : okCount === 0 ? "error" : "partial";
  ...
  await setSettings({
    ...
    lastFullPayload: primaryPayload,
    lastSummary: {
    ...
    const result = await postToDestination(
      {
        ...
        synced_at: new Date().toISOString(),
        message:
          "Hello from Unity Nodes Earnings Tracker — this is a test ping.",
      },
      settings
    );
...
async function addManualAccount({ label, manualToken }) {
  ...
  if (!tok) {
    return {
      ok: false,
      error: "A manual token is required to add an account.",
    };
  }
  ...
  if (existingIdx >= 0) {
    ...
    return {
      ok: true,
      account_id: accounts[existingIdx].id,
      updated: true,
    };
  }
...
async function rescheduleAlarm() {
  ...
  if (s.autoSyncMode === "daily") {
    ...
    if (when) {
      ...
      await setSettings({
        nextScheduledAt: new Date(when).toISOString(),
      });
    } else {
      await setSettings({
        nextScheduledAt: null,
    ...
    await setSettings({
      nextScheduledAt: new Date(Date.now() + 60000).toISOString(),
    });
  }
}
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  if (!result.ok || result.status === "partial") {
    try {
      const errMsg = (
        result.error ||
        result.summary?.errors?.map((e) => e.error).join("; ") ||
        "Sync failed."
      ).slice(0, 200);
    ...
    try {
      switch (msg?.type) {
        ...
        case "ADD_MANUAL_ACCOUNT": {
          const result = await addManualAccount({
            label: msg.label,
            manualToken: msg.manualToken,
          });
